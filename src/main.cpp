...
unsigned int nStakeMaxAge = 7 * 24 * 60 * 60 * 3; // 3 weeks
unsigned int nModifierInterval = 10 * 60; // 10 minutes (time to elapse before new modifier is computed)

int nCoinbaseMaturity = 490; // 490 blocks for coins to mature
...
const string strMessageMagic = "BluntCoin Signed Message:\n";
...
// TODO: update by dec 25, 2014
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    int64_t nSubsidyLimit = 420 * COIN;
...
static const int64_t nTargetTimespan = 14 * 60;  // 14 mins
//
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    printf("AddToBlockIndex() : Rejected by stake modifier checkpoint height=%d, pindexNew->nStakeModifierChecksum=0x%08"PRIx64"\n", pindexNew->nHeight, pindexNew->nStakeModifierChecksum);
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BluntCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (fTestNet)
    {
        ...
        nStakeMinAge = 10 * 60; // test net min age is 10mins
        nModifierInterval = 2 * 60; // test modifier interval is 2 mins
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Dec 3, 2014 - http://www.hightimes.com/read/must-hear-congressman-delivers-most-epic-pro-pot-speech?utm_source=feedburner&utm_medium=feed&utm_campaign=Feed%3A+HIGHTIMESMagazine+(HIGH+TIMES+Magazine)";
        CTransaction txNew;
        txNew.nTime = 1417682509;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 49283 << CBigNum(3142) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xf0eccd575ed31f5ec71666d6bbb53cf9f0358e3c71f50f4e7a02b61783438d48"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc7, 0x9d, 0xa7 };
