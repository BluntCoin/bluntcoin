    ...
    ui(new Ui::ProfitExplorerPage)
{
    ...
    updateTimer.setInterval(188000); // every 188 seconds (approx 2 blocks)
    updateTimer.start();
}
...
void ProfitExplorerPage::loadStakeChart(bool firstRun)
{
    ...
    ui->customPlot->graph(0)->setPen(QPen(QColor(242, 202, 128))); // line color cream for first graph
    ui->customPlot->graph(0)->setBrush(QBrush(QColor(242, 202, 128, 20))); // first graph will be filled with translucent cream
    ui->customPlot->addGraph();
    ui->customPlot->graph(1)->setPen(QPen(QColor(255, 137, 0))); // line color orange for second graph
    ui->customPlot->graph(1)->setBrush(QBrush(QColor(255, 137, 0, 20)));
    ...
    ui->customPlot->xAxis->setTickLabelColor(QColor(191, 138, 73));
    ui->customPlot->yAxis->setTickLabelColor(QColor(191, 138, 73));

    ui->customPlot->rescaleAxes();

    ui->customPlot->xAxis->setLabelColor(QColor(191, 138, 73));
    ui->customPlot->yAxis->setLabelColor(QColor(191, 138, 73));
    ui->customPlot->yAxis->setLabel("BLUNT Minted");
    ...
    ui->difficultyPlot->graph(0)->setPen(QPen(QColor(255, 137, 0))); // line color orange for first graph
    ui->difficultyPlot->graph(0)->setBrush(QBrush(QColor(255, 137, 0, 20))); // first orange will be filled with translucent green
    ...
    ui->difficultyPlot->xAxis->setTickLabelColor(QColor(191, 138, 73));
    ui->difficultyPlot->yAxis->setTickLabelColor(QColor(191, 138, 73));

    ui->difficultyPlot->rescaleAxes();
    ui->difficultyPlot->yAxis->setTickStep(0.00005);
    ui->difficultyPlot->xAxis->setLabelColor(QColor(191, 138, 73));
    ui->difficultyPlot->yAxis->setLabelColor(QColor(191, 138, 73));
    ...
    ui->velocityPlot->graph(0)->setPen(QPen(QColor(255, 137, 0))); // line color orange for first graph
    ui->velocityPlot->graph(0)->setBrush(QBrush(QColor(255, 137, 0, 20))); // first graph will be filled with translucent orange
    ...
    ui->velocityPlot->xAxis->setTickLabelRotation(18);

    ui->velocityPlot->xAxis->setTickLabelColor(QColor(191, 138, 73));
    ui->velocityPlot->yAxis->setTickLabelColor(QColor(191, 138, 73));

    ui->velocityPlot->yAxis->setScaleType(QCPAxis::stLogarithmic);
    ui->velocityPlot->yAxis->setTickStep(1000);
    ui->velocityPlot->xAxis->setLabelColor(QColor(191, 138, 73));
    ui->velocityPlot->yAxis->setLabelColor(QColor(191, 138, 73));
    ui->velocityPlot->yAxis->setLabel("BLUNT");
